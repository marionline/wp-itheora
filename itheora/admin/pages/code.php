...
<select name="s" >
<?php
$rep = $document_root."/skins/"; $dir = opendir($rep); // Fix path for wp-itheora 
 ...
 */
	echo "<option value=\"".$lg."\" selected=\"selected\">".$lg." auto</option>";

/**
 * Fix bug to display language in creation player, to inform itheora project 
 */
while ($f = readdir($dir)) {
	if(is_dir($rep.$f) && ($f!="." && $f!=".." && $f!=$lg)) {
		echo "<option value=\"".$f."\">".$f."</option>";
	}
}
closedir($dir);
...
<p class="indent"><input type="submit" class="itheora-submit" value="<?php echo txt($c_ok)?>" /></p>
</div>
</form>
...
if ($handle = @fopen($document_root.'/skins/'.$_POST['s'].'/style.php', "r")) {
...
if(isset($_POST['v'])) {
	if(file_exists($document_root."/data/".$_POST['v'])) {
