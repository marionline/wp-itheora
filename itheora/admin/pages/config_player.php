<?php 
// require_once("../config/player.php"); <--- included in wp-itheora plugin
// require_once("../lib/fonctions.php"); <--- not need in wp-itheora, just included
...
<form action="'.$_SERVER['REQUEST_URI'].'" method="post"><div>
	...
	<br />
<p class="indent"><input type="submit" name="save" class="itheora-submit" value="'.txt($cp_save).'" /></p>
</div></form>
';
}
...
if($save==txt($cp_save)) {
	...
	$old_file_config_player= fopen($document_root."/admin/config/player.php","w");
